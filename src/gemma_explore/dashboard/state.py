...
def fig_to_png_bytes(figure: Figure, dpi: int = 100) -> bytes:
    buf = BytesIO()
    try:
...
def png_bytes_to_html(png_bytes: bytes, width: str = "100%") -> str:
...
class DashboardState:
    ...
    freq_scores: Optional[dict] = None
    ...
    def get_hidden_png(self, layer: int, head: int, builder: Callable[[], Figure]) -> bytes:
        ...
        if key not in self._hidden_png:
            self._hidden_png[key] = fig_to_png_bytes(builder())
